...
def generate_sen12_json(root_dir: Path, output_dir: Path,
...
def generate_qxs_json(root_dir: Path, output_dir: Path,
                      split_ratio: Tuple[float, float, float]=(0.8, 0.1, 0.1), 
                      seed: int=114514):
    ...
    sar_dir = root_dir / 'sar_256_oc_0.2'

    image_list = sorted(sar_dir.glob('*.png'))
    ...
    with open(output_dir / 'qxs.json', 'w', encoding='utf-8') as f:
        json.dump(dataset, f, indent=4)
        
    print(f'Generated QXS dataset json at {output_dir / "qxs.json"}')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Generate dataset JSON for SEN12 or QXS.")
    
    parser.add_argument('--root_dir', '-r', type=Path, required=True, help='Root directory of the dataset')
    parser.add_argument('--json_dir', '-o', type=Path, required=True, default=Path('./data'), help='Output directory to save generated JSON files')
    parser.add_argument('--dataset',  '-d', type=str,  required=True, choices=["sen12", "qxs"], help="Dataset type")
    ...
    assert root_dir.exists(),   'Dataset not found'
    assert output_dir.exists(), 'SEN12 Output directory not found'
    
    if dataset == 'sen12':
        generate_sen12_json(root_dir, output_dir)
        
    elif dataset == 'qxs':
        generate_qxs_json(root_dir, output_dir)