...
def train(args: DictConfig):
    ...
    patchgan_loss     = losses.PatchGANLoss(in_channels=sar_shape[0]+opt_shape[0]).to(device)
    ...
    optimizer_D = optim.Adam(patchgan_loss.discriminator.parameters(), lr=args.train.patchgan_lr)
    ...
    for epoch in range(current_epoch, args.train.epochs+1):
        ...
        train_loss_gan_D        = 0.0
        train_loss_gan_G        = 0.0
        ...
        for i, (sar, opt) in train_bar:
            ...
            train_loss_gan_G        += loss_gan_G.item()
            train_loss_gan_D        += loss_gan_D.item()
            
            train_bar.set_postfix(
                # loss  = f'{train_loss/(i+1):.4f}',
                v=f'{train_loss_velocity/(i+1):.3f}',
                i=f'{train_loss_image/(i+1):.3f}',
                l=f'{train_loss_lpips/(i+1):.3f}',
                d=f'{train_loss_distillation/(i+1):.3f}',
                s=f'{train_loss_ssim/(i+1):.3f}',
                gg=f'{loss_gan_G:.3f}',
                gd=f'{loss_gan_D:.2f}'
            )
        ...
        avg_train_loss_gan_G        = train_loss_gan_G        / len(train_loader)
        avg_train_loss_gan_D        = train_loss_gan_D        / len(train_loader)
        
        logger.info(f'[Train] Epoch {epoch}: loss {avg_train_loss:.4f}, vel_loss {avg_train_loss_velocity:.4f}, lpips_loss {avg_train_loss_lpips:.4f}, dist_loss {avg_train_loss_distillation:.4f}, ssim_loss {avg_train_loss_ssim:.4f}, img_loss {avg_train_loss_image:.4f}, gan_G_loss {avg_train_loss_gan_G:.4f}, gan_D_loss {avg_train_loss_gan_D:.4f}')
        ...
        with torch.no_grad():
            ...
            for i, (sar, opt) in valid_bar:
                ...
                valid_bar.set_postfix(i=f'{valid_loss_image/(i+1):.4f}',
                                      ...
                                      p=f'{valid_psnr/(i+1):.2f}', 
                                      l=f'{valid_lpips/(i+1):.4f}', 
                                      s=f'{valid_ssim/(i+1):.4f}')
